"""
Western Music & Scene — Vibe Signal Collector
-----------------------------------------------
서양 음악·씬 소스에서 Vibe 후보를 수집.
DISCORD_EN_MUSIC_WEBHOOK
스코어링: Vibe & Signal 밀도 5지표

환경변수:
  ANTHROPIC_API_KEY         Claude API 키
  DISCORD_EN_MUSIC_WEBHOOK  Discord 웹훅
"""
...
SUMMARY_SYSTEM_PROMPT = (
    "당신은 서양 음악·씬 Vibe 신호 분석가입니다. "
    "200자 이내 2~3문장. 레이블 없이 이어서. 씬·평론·정체성·흐름 관점. 본문에 있는 내용만. 한국어."
)
...
def main() -> None:
    ...
    webhook_url = os.environ.get("DISCORD_EN_MUSIC_WEBHOOK")

    if not api_key:
        raise EnvironmentError("ANTHROPIC_API_KEY 환경변수가 설정되지 않았습니다.")
    if not webhook_url:
        raise EnvironmentError("DISCORD_EN_MUSIC_WEBHOOK 환경변수가 설정되지 않았습니다.")
